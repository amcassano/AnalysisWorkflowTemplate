...
```{r install_pkgs, eval=FALSE, message=FALSE, warning=FALSE}
...
# for this line of code the only thing you need to do is change `one_vs_two` to be one of your pairwise comparisons
...
# change GMattributes if desired, be warned though if you add EntrezID you'll need to adjust things further down to reflect that
# If you wnat to do KEGG analysis you will need the EntrezIDs, i would recommend creating 2 annotated dataframes for ease of future use
```
...
three_comp_venndiagram <- analyzeRNA::plot_venndiagram(list("One" = sig_OneThree$MGI_Symbol,
...
overlaps_onetwofour_vsthree <- analyzeRNA::venn_overlaps(list("One" = sig_OneThree$MGI_Symbol,
                                                              "Two" = sig_TwoThree$MGI_Symbol,
                                                              "Four" = sig_FourThree$MGI_Symbol))
```
...
analyzeRNA::revigoPlot(gsea_obj = one_vs_two_GSEA,
...
go_groups2 <- analyzeRNA::goGrouping(one_vs_two, pval_cutoff = 0.05, l2fc_cutoff = 0.45, lvl = 2) %>% tibble::remove_rownames()

go_groups3 <- analyzeRNA::goGrouping(one_vs_two, pval_cutoff = 0.05, l2fc_cutoff = 0.45, lvl = 3) %>% tibble::remove_rownames()

go_groups4 <- analyzeRNA::goGrouping(one_vs_two, pval_cutoff = 0.05, l2fc_cutoff = 0.45, lvl = 4) %>% tibble::remove_rownames()
