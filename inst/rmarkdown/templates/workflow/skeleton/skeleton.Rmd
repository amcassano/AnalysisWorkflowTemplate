...
*Built with R version 4.3.1. Most recent template changes made on January 10, 2024*
...
-   **Pairwise comparison function inputs:**
    ...
    -   `numerator`-\> required; string; must match exactly as it's written in the DESeq object.
        If unsure run just the `resultsNames(dseq)` line of code.
        (ex: "Tolerant", "Naive")
...
gmap <- analyzeRNA::createGenemap(allgenes,
                                  GMattributes = c("ensembl_gene_id", "mgi_symbol", "mgi_description")) #change this if desired
```
...
one_vs_two <- analyzeRNA::annotate_biomart(one_vs_two,
                                           ...
                                           gmattri = c("MGI_Symbol", "MGI_Desc")) #change this if you changed whats in the annotation
...
-   **Filter significant DEGs function inputs**
    ...
    -   `entrezid` -\> optional; boolean; defaults to `TRUE`, change to `FALSE` if EntrezID is not in your annotations
...
ranked_OneTwo <- analyzeRNA::signif_deg(one_vs_two,
                                        padj_threshold,
                                        l2fc_threshold,
                                        entrezid = F)
ranked_TwoThree <- analyzeRNA::signif_deg(two_vs_three,
                                          padj_threshold,
                                          l2fc_threshold,
                                          entrezid = F)
...
-   **Export function inputs**
    ...
    -   `filename` -\> required; string; the name of the file to export to, it will be exported to your cwd set above, do not include ".csv" that will be added automatically, if you're exporting only up- or down-regulated genes that will be added to the file name automatically as well
...
analyzeRNA::export_genelist(ranked_OneTwo,
                            filename = "1vs2",
                            direction = "up")
...
-   **PCA inputs**
    ...
    -   `dseq_transform` -\> required; DESeqTransform object; must be the actual object not the dataframe (ex. `rlog_norm`)
...
```{r plot_counts, eval=TRUE}
#edit this so that its the right comparisons
statcomps <- list(c("Naive", "Tolerant"),
                  c("Naive", "Rejection"),
                  c("Tolerant", "Rejection"))

plot_genecounts("Pdcd1",
                rlog_df,
                meta,
                plot_aes)

plot_genecounts(mgi = "Tox",
                norm_df = rlog_df,
                metadat = meta,
                plot_aes =  plot_aes,
...
+----------------------------+-------------------------------+-------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `dircolors`                | Optional;                     | `c("green", "red", "gray70")` | The colors for upregulated in condition 1, downregulated in condition 1 and not significant. I recommend leaving the third as `"gray70"` or gray with any number 70-90. You can copy paste the exact color id from your plot aes dataframe if you want it to match. |
|                            |                               |                               |                                                                                                                                                                                                                                                                     |
...
| Input                                               | Required?                   | Example/Notes                                                                                                                                     |
+=====================================================+=============================+===================================================================================================================================================+
| `deg_df` dataframe of annotated pairwise comparison | Required                    | Use the annotated pairwise comparison, not the filtered one and not the rlog counts                                                               |
+-----------------------------------------------------+-----------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------+
| `pval_cutoff` number                                | Optional; defaults to 0.05  |                                                                                                                                                   |
+-----------------------------------------------------+-----------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------+
| `l2fc_cutoff` number                                | Optional; defaults to 0.5   |                                                                                                                                                   |
+-----------------------------------------------------+-----------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------+
| `ontol` string                                      | Optional; defaults to "ALL" | One of `"ALL"`, `"BP"`, `"CC"`, or `"MF"` these mean including all ontologies, just biological processes, cell components, or molecular functions |
+-----------------------------------------------------+-----------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------+
...
| Input            | Required?                     | Example/Notes                                                                                                                                     |
+==================+===============================+===================================================================================================================================================+
| `degs` dataframe | Required                      | annotated pairwise comparison dataframe (not filtered)                                                                                            |
+------------------+-------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------+
| `ontol` string   | Optional; defaults to `"ALL"` | One of `"ALL"`, `"BP"`, `"CC"`, or `"MF"` these mean including all ontologies, just biological processes, cell components, or molecular functions |
+------------------+-------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------+
...
one_vs_two_GSEA <- go_GSEA(degs = one_vs_two, pval_cutoff = 0.075, ontol = "BP")
one_vs_two_ORA <- go_ORA(deg_df = one_vs_two, pval_cutoff = 0.075, ontol = "BP")
one_vs_two_FC <- getFoldChanges(one_vs_two)
one_vs_two_sigFC <- getFoldChanges(ranked_one_vs_two)
```
...
gseRidgePlot <- makeGSEplot(GSE_obj = one_vs_two_GSEA,
                            plot_title = "1 vs 2 GSEA",
                            plot_type = "ridge")
...
gseClusterPlot <- makeGSEplot(GSE_obj = one_vs_two_GSEA,
                              plot_title = "1 vs 2 GSEA",
                              plot_type = "cluster")
...
gseHeatPlot <- makeGSEplot(GSE_obj = one_vs_two_GSEA,
                           plot_title = "1 vs 2 GSEA",
                           plot_type = "heat")
...
gseUpsetPlot <- makeGSEplot(GSE_obj = one_vs_two_GSEA,
                            plot_title = "1 vs 2 GSEA",
                            plot_type = "upset")
...
gseTreePlot <- makeGSEplot(GSE_obj = one_vs_two_GSEA,
                           plot_title = "1 vs 2 GSEA",
                           plot_type = "tree")
...
gseTradPlot <- makeGSEplot(GSE_obj = one_vs_two_GSEA,
                           plot_title = "1 vs 2 GSEA",
...
catplot <- categorynetplot(go_obj = one_vs_two_GSEA,
                           deg_df = one_vs_two,
                           pval_cutoff = 0.01, #defaults to 0.05
                           l2fc_cutoff = 0.8, #defaults to 0.5
                           categories_to_show = 35, #defaults to 30
                           shape = "circle") #defaults to "circle"
...
oxphos_heatmap <- heatmapFromCSV(plottitle = "Oxidative Phosphorlyation Genes",
                                 fname = paste(cwd, "oxphos.csv", sep = "/"),
                                 met = meta,
                                 counts_df = rlog_df,
                                 ...
                                 height = 8) #defaults to 10, decreasing will make a more squished map, better for lots of genes

oxphos_heatmap
```

### Heatmap from multiple CSVs

```{r multiCSV_heatmap, eval=TRUE}
#first create a dataframe consolidating all the genelists of interest
cellprolif <- consolidate_gene_list(map = gmap,
                                    paste(cwd, "cell_division.csv", "/"),
                                    paste(cwd, "cell_proliferation.csv", "/"),
                                    paste(cwd, "nuclear_division.csv", "/"),
                                    paste(cwd, "chromosome_segregation.csv", "/"))

prolif_heatmap <- heatmapFromDF(plottitle = "Cell Division and Proliferation",
                                genelist = cellprolif,
                                met = meta,
                                counts_df = rlog_df,
                                annocolors = anno_colors,
                                ...
                                height = 8) #defaults to 10

prolif_heatmap
```

### Heatmap from some top genes

```{r df_heatmap, eval=TRUE}
#first grab the top however many genes from your comparisons
# below shows how to get the top genes from 2 different comparisons together.
#you can add or subtract depending on your comparisons
top_genes <- rbind(
  dplyr::slice_head(ranked_OneTwo, n = 15) %>% select(MGI_Symbol),
  dplyr::slice_head(ranked_TwoThree, n = 15) %>% select(MGI_Symbol)
)
top_genes <- dplyr::distinct(top_genes, MGI_Symbol, .keep_all = TRUE)

top_heatmap <- heatmapFromDF(plottitle = "Top 15 genes from each comparison",
                             genelist = top_genes,
                             met = meta,
                             counts_df = rlog_df,
                             annocolors = anno_colors)
...
```{r go_heatmaps}
go_groups2 <- goGrouping(one_vs_two, pval_cutoff = 0.075, l2fc_cutoff = 0.45, lvl = 2) %>% print()

go_groups3 <- goGrouping(one_vs_two, pval_cutoff = 0.075, l2fc_cutoff = 0.45, lvl = 3)

go_groups4 <- goGrouping(one_vs_two, pval_cutoff = 0.075, l2fc_cutoff = 0.45, lvl = 4)


f <- getgenelistfromgogroup(go_groups2, 1)
heatmapFromDF("GO Group", f, meta, rlog_df, anno_colors, gaps = c(5), height = 8)
```
