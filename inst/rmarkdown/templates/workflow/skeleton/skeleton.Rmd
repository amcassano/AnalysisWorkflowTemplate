...
BiocManager::install("pathfindr.data")
...
[**Change the group names and number of samples**]{style="color: red"}
...
# pre-filtering
num_samples <- 10
raw_counts <- analyzeRNA::prefilterCounts(raw_counts, num_samples, avg_count_minimum = 200, stringency = 2)
...
The code above will allow you to plot one MGI symbol at a time.
If you want to create plots for many different genes at once use this chunk To do this, create a list of MGI symbols and the code will generate plots for all genes listed and run the mapping code to do all at once.
The other arguments in the plot will be the same for all genes in the list.
If you want to change around the stat comparisons listed for each of the plots check the chunk below this one
...
If you want to have different stats for pairwise comparisons listed for different genes that you're plotting you'll use this You will create a list of MGI symbols the same way as above but you'll also create a separate list of comparisons.
...
```{r twoway_venn_diagrams}
# if you *don't* want the images exported to pdf, comment the line below and the final line of the chunk out. You can change the size of the images if you'd like
grDevices::pdf(file = paste0(output_dir, "/twowayvenndiagrams.pdf"))

# First you'll create a list of the comparisons that you want in your venn diagram
# the list has to be a named list to associate the name of the comparison with the list of genes you're inputing
# you will be taking the MGI symbol from your filtered list of significant genes in a pairwise comparsion
twoway_venn_inputs <- list(
  "One vs Three" = sig_OneTwo$MGI_Symbol,
  "Two vs Three" = sig_TwoThree$MGI_Symbol
)

# if you want the circle edges to be color coded you can also create a list with colors below
twoway_venn_colors <- c(group1_color, group2_color)

analyzeRNA::plot_venndiagram(
  degs_list = twoway_venn_inputs,
  plt_title = "# DEGs shared vs. Three",
  edgecolor = twoway_venn_colors
)
...
This chunk will help you make venn diagrams comparing the number of shared DEGs between three sets of pairwise comparisons.

```{r threeway_venndiagrams}
# if you *don't* want the images exported to pdf, comment the line below and the final line of the chunk out. You can change the size of the images if you'd like
grDevices::pdf(file = paste0(output_dir, "/threewayvenndiagrams.pdf"))

# First you'll create a list of the comparisons that you want in your venn diagram
# the list has to be a named list to associate the name of the comparison with the list of genes you're inputing
# you will be taking the MGI symbol from your filtered list of significant genes in a pairwise comparsion
threeway_venn_inputs <- list(
  "One vs Three" = sig_OneTwo$MGI_Symbol,
  "Two vs Three" = sig_TwoThree$MGI_Symbol,
  "Four vs Three" = sig_FourThree$MGI_Symbol
)

# if you want the circle edges to be color coded you can also create a list with colors below
threeway_venn_colors <- c(group1_color, group2_color, group4_color)


analyzeRNA::plot_venndiagram(
  degs_list = threeway_venn_inputs,
  plt_title = "# DEGs shared vs. Three",
  edgecolor = threeway_venn_colors
)
...
If you want to get a list of all the genes in any given section of the venn diagrams you created you'll use this code You can get either the overlaps (what's shared) or the unique unshared genes
