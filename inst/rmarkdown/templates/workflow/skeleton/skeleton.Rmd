...
|----------------|----------------|----------|-------------------------------|
| `numerator` string        | Required                        | `"Tolerant"` `"Naive"` etc | Must match exactly the way it's written in the DESeq object. If unsure run just the `resultsNames(dseq)` line of code.                |
| `denominator` string      | Required                        | `"Tolerant"` `"Naive"` etc | Must match exactly the way it's written in the DESeq object. If unsure run just the `resultsNames(dseq)` line of code.                |
| `dseq_obj` DESeqObject    | Required                        | dseq                       | This is the DESeq object we saved two chunks above. Don't change the name of it unless you also changed the name in the above chunks. |
| `contrast_str` string     | Optional; defaults to `"Group"` |                            |                                                                                                                                       |
| `tidy_result` TRUE/FALSE  | Optional; defaults to `TRUE`    |                            |                                                                                                                                       |
| `log2FCshrink` TRUE/FALSE | Optional; defaults to `FALSE`   |                            |                                                                                                                                       |
...
| Input                     | Required?                                      | Example                                    | Notes                                                                                                                                |
|------------|------------|----------------|------------------------------------|
| `res` results dataframe   | Required                                       | `rlog_df`, a pairwise comparison dataframe | must include 'GeneID' column with ensembl ID                                                                                         |
| `gm` genemap              | Required                                       | `gmap`                                     | created using the chunk above                                                                                                        |
| `gmattri` list of strings | Optional; defaults to MGI symbol & description | `c("MGI_Symbol", "MGI_Desc", "GeneType")`  | Dictates which columns from the gene map are kept after renaming and reordering. Can't include a column not present in the genemap.  |
...
| `l2fc_cutoff` number         | Optional; defaults to 0.8  |
...
| `tab`       | Required                   |         |       |
| `filename`  | Required                   |         |       |
| `direction` | Optional; defaults to both |         |       |
|             |                            |         |       |
|             |                            |         |       |
...
-   MGI symbol for the gene of interest,
-   annotated data frame of rlog annotated counts
-   the metadata data frame
-   plot aesthetics.

Optional Inputs:

-   a list of comparisons to get p values for. The format for this is \`comparisons = list(c("condition1", "condition2"), c("condition2", "condition3")) adding as many comparisons as desired. The order of comparisons in the list will correspond to the order in which the pvalues are plotted on the graph.
...
-   you can change the location of the kruskal wallis statistic that will be included using `kw_hjust = yournumberhere` and `kw_vjust = yournumberhere`
